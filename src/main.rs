...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        tokio::time::sleep(tokio::time::Duration::from_secs(
            config.dynamic_fee_update_frequency,
        ))
        .await;
    }
}
...
async fn process_channels(
    client: Arc<Mutex<tonic_lnd::Client>>,
    channels: Vec<tonic_lnd::lnrpc::Channel>,
) {
    ...
    channels_stream
        .for_each_concurrent(None, move |c| {
            ...
            async move {
                ...
                let channel_point = tonic_lnd::lnrpc::ChannelPoint {
                    output_index: output_index_str
                        .parse()
                        .expect("Output index should be a valid u32 number"),
                    funding_txid: Some(
                        tonic_lnd::lnrpc::channel_point::FundingTxid::FundingTxidStr(
                            funding_txid_str.to_string(),
                        ),
                    ),
                };
                ...
                let request = tonic_lnd::lnrpc::PolicyUpdateRequest {
                    scope: Some(tonic_lnd::lnrpc::policy_update_request::Scope::ChanPoint(
                        channel_point,
                    )),
                    base_fee_msat: 1000,
                    fee_rate,
                ...
                let result = client_clone
                    ...
                    .await
                    .expect("failed to update channel policy");
                debug!("{:?}", result);
            }
        })
